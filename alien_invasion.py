...
class AlienInvasion:
    ...
    def _create_fleet(self):
        ...
        number_aliens_x = available_space_x // (2 * alien_width)  # //为整除运算符，将两个数相除并舍弃余数
    ...
    def _create_alien(self, alien_number, row_number):
        alien = Alien(self)
        alien_width, alien_height = alien.rect.size  # size属性是一个元组，包含对象的宽度和高度
    ...
    def _update_aliens(self):
        ...
        self.aliens.update()  # 对编组调用update，将会对编组内每个外星人调用update方法

        # 检测外星人和飞船之间的碰撞
        if pygame.sprite.spritecollideany(self.ship,self.aliens):   # 找到以一个与飞船碰撞的外星人。 入伙没有返回None
            self._ship_hit()
        # 检查外星人是否到达屏幕底端
    ...
    def _update_screen(self):
        self.screen.fill(self.settings.bg_color)  # 屏幕背景颜色
        self.ship.blitme()
